...
Approval blocks:
Editing this selected Things task documents work; it does not execute the actions described in its title or notes. If the receiver blocks a write, preserve the proposed edit and report the actual rejection and affected fields. Use the receiver's supported human-approval mechanism if it is available for this run and permits escalation. Show the exact task and proposed changes, and wait for approval before resuming. If no approval path is available, end with "Enrichment incomplete: approval required" and show the proposed edit in the private run result. Do not silently skip notes, repeatedly resubmit denied calls, weaken review settings, change wording to evade review, or use another tool to perform the blocked write. A refusal is not a transport retry. Do not assume a blocked call reached MCP; if execution is uncertain, inspect the receipt and read back before any further write. Report fields already applied separately from fields awaiting approval. After approval, reconcile current state and apply only authorized outstanding changes using the tool's request-ID and recovery rules.
...
failure. Version 0.10.4 incorrectly classified native incremental text patches as
rich notes and refused replacement. The corrected reader reconstructs verified
patches before editing; unsupported or invalid note data remains unavailable.
A screenshot alone cannot establish which server version handled a run.
