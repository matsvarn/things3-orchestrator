...
def _chmod(path: Path, mode: int, *, descriptor: int | None = None) -> None:
...
def _atomic_replace(
    ...
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=prefix or f".{path.name}.", suffix=".tmp", dir=path.parent
    )
    temporary = Path(temporary_name)
    payload = data.encode("utf-8") if isinstance(data, str) else data
    try:
        _chmod(temporary, 0o600, descriptor=descriptor)
