...
Materialized store indexes Task3/4/6, Area2/3, Tag3/4, and ChecklistItem/2/3. Checklist items attach to the parent task and sort by `ix`. Headings (`tp=2`) are dropped from most listings. Recurrence templates (`rr` set and `rt` empty) are dropped from Someday. Markable IDs are non-trashed, non-heading `Task6` rows. Tombstone entities are excluded from short-ID domain. ([src/store/mod.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/store/mod.rs), [src/store/entities.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/store/entities.rs))
...
Current write entity names: `Task6`, `Area3`, `Tag4`, `ChecklistItem3`. Updates use `EntityType::from(existing.entity)` so a Task UUID is not sent as `Area3` and an Area UUID is not sent as `Task6`. `mark` refuses non-`Task6` tasks. ([src/wire/wire_object.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/wire/wire_object.rs), [src/commands/edit.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/edit.rs), [src/commands/mark.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/mark.rs), [src/commands/delete.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/delete.rs))
...
New tasks are `t=0` `e=Task6` with `ThingsId::random()`, `cd`/`md` = now, `xx: {"_t":"oo","sn":{}}`, and notes only if non-empty. Empty notes stay `nt: null` via struct default. Inbox is `st=0`. Project/area container forces `st=1` plus `pr` or `ar`. Today is `st=1` + `sr`/`tir` = today UTC midnight. The pinned client converts a future `--when YYYY-MM-DD` to local midnight. A live Berlin probe showed that Things displays the UTC calendar day instead. The local conversion therefore displayed both start and deadline one day early. This adapter uses UTC midnight for `sr`, `tir`, `rmd`, and `dd`. Deadline is `dd`. Sibling insert uses `ix` gap math (midpoint, or rebalance to stride 1024). ([src/commands/new.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/new.rs), [src/cmd_ctx.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/cmd_ctx.rs), [src/wire/task.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/wire/task.rs))
...
Projects are also `Task6` with `tp=1`. Areas are `Area3` `{tt, tg, ix, xx}`. Tags are `Tag4` `{tt, sh, ix, pn, xx}`. Delete is `t=2` with the object's entity name and empty `p` (not a `Tombstone2` create). ([src/commands/projects.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/projects.rs), [src/commands/areas.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/areas.rs), [src/commands/tags.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/tags.rs), [src/commands/delete.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/delete.rs), [src/wire/area.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/wire/area.rs), [src/wire/tags.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/wire/tags.rs), [src/wire/tombstone.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/wire/tombstone.rs))
...
- Never send `Task6` for an Area UUID; updates keep the stored entity name. ([src/commands/edit.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/edit.rs), [src/commands/projects.rs](https://github.com/evanpurkhiser/things3-cloud/blob/1281f43bc677325968a6fdea242a5c39bb04d208/src/commands/projects.rs))
...
- Full `Task6` create (`xx` conflict object, empty arrays, `nt` null when empty). Keep Base58 IDs.
- Notes `{_t:"tx", ch:crc32, v, t:1}` (we already do this).
- Preserve stored `e` on updates (`Task6` vs `Area3`). Filter `tp=2` and `rr` templates from listings.
