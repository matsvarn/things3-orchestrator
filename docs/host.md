...
All clients share one MCP bearer. There is no per-client identity. Possession
of that bearer authorizes every bounded v2 mutation, including recoverable
Trash and repeat Stop. Keep it private and give it only to agents that may
write to this Things account.

An agent or plugin with arbitrary code execution as the serving OS user can
also replace the server or journal. Run untrusted agent runtimes under a
different OS user from the serving process, or treat their host access as full
write authority.
